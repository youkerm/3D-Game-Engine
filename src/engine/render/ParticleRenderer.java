...
import engine.render.shader.ParticleShader;
...
public class ParticleRenderer {
	...
	public ParticleRenderer(GLLoader loader, Matrix4f projectionMatrix){
		this.loader = loader;
	...
	private void updateModelViewMatrix(Vector3f position, float rotation, float scale, Matrix4f viewMatrix, float[] vboData) {
		...
		modelMatrix.translate(position, modelMatrix, modelMatrix);
		...
		modelMatrix.rotate((float) Math.toRadians(rotation), new Vector3f(0, 0, 1), modelMatrix, modelMatrix);
		modelMatrix.scale(new Vector3f(scale, scale, scale), modelMatrix, modelMatrix);
